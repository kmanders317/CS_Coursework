...
def about(subject):
    ...
    def words_func(paragraph):
        values = split(paragraph)
        for i in values:
            if lower(remove_punctuation(i)) in subject:
                    return True
        return False
    return words_func
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for w in range(len(word_list)):
        if diff_function(typed_word, word_list[w], limit) < diff:
            diff = diff_function(typed_word, word_list[w], limit)
            word = word_list[w]
...
def feline_fixes(typed, source, limit):
    ...
    5
    """
    def help(i, count, typed, source, limit):
        if count > limit:
            return limit + 1
        if len(typed) > len(source):
            return help(i, count, typed[0:len(source)], source, limit) + abs(len(source)-len(typed))
        if len(typed) < len(source):
            return help(i, count, typed, source[0:len(typed)], limit) + abs(len(source)-len(typed))
        if typed[i] != source[i]:
            count += 1
        if i == (len(typed) - 1):
            return count
        return help(i+1, count, typed, source, limit)
    if typed == source:
        return 0
    else:
        return help(0, 0, typed, source, limit)
...
def final_diff(typed, source, limit):
    ...
    assert False, 'final_diff function.'
...
def report_progress(typed, source, user_id, upload):
    ...
    up = {'id': user_id, 'progress': progress}
    upload(up)
    return progress
...
def time_per_word(words, timestamps_per_player):
    ...
    while i < len(timestamps_per_player):
        time_count = 1
        small_list = list()
        while time_count < len(timestamps_per_player[i]):
            small_list = small_list + [timestamps_per_player[i][time_count] - timestamps_per_player[i][time_count-1]]
            time_count += 1
        time_list = time_list + [small_list]
        i += 1
...
def fastest_words(match):
    ...
    for p in player_indices:
        ...
        for i in word_indices:
            player_time = time(match, p, i)
            lowest = player_time
            for t in player_indices:
                if time(match, t, i) < lowest:
                    lowest = time(match, t, i)
            if lowest == player_time:
                if p == 0:
                    player_list = player_list + [get_word(match, i)]
                else:
                    already = False
                    for z in range(p):
                        if get_word(match, i) in full_list[z]:
                            already = True
                    if already == False:
                        player_list = player_list + [get_word(match, i)]
        full_list = full_list + [player_list]
    return full_list
